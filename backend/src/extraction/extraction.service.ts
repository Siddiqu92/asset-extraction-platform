      ...
      if (needsAi) {
        try {
          ...
          for (let i = 0; i < chunksToProcess.length; i++) {
            ...
            // FIX: pass sourceFile as fileName (second argument)
            const openAiRaw = await this.aiService.extractWithOpenAI(chunksToProcess[i], sourceFile);
            this.logger.log(`OpenAI chunk ${i + 1}: ${openAiRaw.length} assets`);
            // FIX: correct argument order — assets first, then text
            const validated = await this.aiService.validateWithClaude(openAiRaw, chunksToProcess[i]);
